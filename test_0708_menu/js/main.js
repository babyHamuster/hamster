$(document).ready(function(){
    /* 
        ...
        1024 이하 mobile
    */
   let pc_mobile //현재 pc인지 mobile 인지 상태를 저장하는 변수
   ...
   resize_chk() //thml이 로딩된 이후 단 1번 실행
   ...
   /*
        ...
        오버한 li에 (header .gnb .gnb_wrap ul.depth1 > li) on 클래스 추가
    */
   $('header .gnb .gnb_wrap ul.depth1 > li').on('mouseenter focusin', function(){
       /* pc일때만 작동되어야함
          on이라는 명령은 이벤트핸들러(감시)로 pc에서만 작동(감시)되게 못함 
          무조건 작동(감시)됨 
          이벤트 발생했을때 작동되는 실행문에 if문을 걸어서 pc일때만 실행되도록 해야함*/
        if(pc_mobile == 'pc'){
    ...
    */
    ...
    /*
        메뉴 열기를 클릭하면 header에 menu_open클래스 추가 
        메뉴 닫기를 클릭하면 header에 menu_open클래스 삭제
